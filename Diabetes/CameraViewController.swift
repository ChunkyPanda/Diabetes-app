...
class CameraViewController: UIViewController {
    ...
    let app = ClarifaiApp(appID: "2plyeEx2CMUP6kfSX3JouiS3M-p1pDrexcVG4wcb", appSecret: "NDnt3hYQ5_NB-PbkueLlFsSZRxQXY_SdDspYeVxc")
            ...
            model?.predict(on: [clarifaiImage!], completion: {(outputs, error) in
                if error == nil {
                    ...
                    DispatchQueue.main.async(execute: {() in
                        ...
                        if tags.count > 2 {
                            self.button1.setTitle((tags[0] as? String), for: .normal)
                            self.button2.setTitle((tags[1] as? String), for: .normal)
                            self.button3.setTitle((tags[2] as? String), for: .normal)

...
struct NutritionData {
    ...
    {
        guard let itemName = json["brand_name_item_name"] as? String else {throw SerializationError.missing("Item name is missing")}
        ...
        guard let servingSize = json["serving_qty"] as? Int else {throw SerializationError.missing("Serving size is missing")}
        
        guard let servingUnit = json["serving_unit"] as? String else {throw SerializationError.missing("Serving unit is missing")}
    ...
    static let basePath = "https://trackapi.nutritionix.com/v2/search/instant?query=big+mac"
    static let appID = "63fb7aa5"
    static let appKey = "d74d7a2712edd8b7319195c72f88390c"
    
    static func queryNutrionix(withInput userInput: String, completion: @escaping ([NutritionData]) -> ()) {
        let url = basePath + userInput
        var request = URLRequest(url: URL(string: url)!)
        request.httpMethod = "GET"
        request.setValue("63fb7aa5", forHTTPHeaderField: "x-app-id")
        request.setValue("d74d7a2712edd8b7319195c72f88390c", forHTTPHeaderField: "x-app-key")
        
        let task = URLSession.shared.dataTask(with: request) { (data: Data?, response: URLResponse?, error:Error?) in
            var nutritionixArray:[NutritionData] = []
            
            if let data = data {
                do {
                    if let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String:Any] {
                        if let branded = json["branded"] as? [[String:Any]] {
                            for dataPoint in branded {
                                if let foodItem = try? NutritionData(json: dataPoint) {
                                    nutritionixArray.append(foodItem)
                                }
                            }
                        }
                    }
...
class CalorieTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = FoodStorageService.sharedInstance.storage[indexPath.row].get_name()
